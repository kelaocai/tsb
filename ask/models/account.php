...
class account_class extends AWS_MODEL {
	...
	function get_user_info_by_uids($uids, $attrib = false) {
		...
		if (sizeof($uids) == 1) {
			if ($one_user_info = $this -> get_user_info_by_uid(end($uids), $attrib)) {
				return array(end($uids) => $one_user_info);
			}
	...
	function user_register($user_name, $password, $email, $mobile, $email_valid = false) {
		if ($uid = $this -> insert_user($user_name, $password, $email,'0', $mobile)) {
			...
			$this -> update_users_fields(array('valid_email' => intval($email_valid), 'group_id' => $group_id, 'reputation_group' => 5, 'invitation_available' => get_setting('newer_invitation_num'), 'is_first_login' => 1), $uid);
	...
	function update_user_name($user_name, $uid) {
		$this -> update('users', array('user_name' => htmlspecialchars($user_name), ), 'uid = ' . intval($uid));
	...
	function clean_first_login($uid) {
		if (!$this -> shutdown_update('users', array('is_first_login' => 0), 'uid = ' . intval($uid))) {
			return false;
		} else {
			return true;
		}
	...
	public function update_notification_setting_fields($data, $uid) {
		if (!$this -> count('users_notification_setting', 'uid = ' . intval($uid))) {
			$this -> insert('users_notification_setting', array('data' => serialize($data), 'uid' => intval($uid)));
		} else {
			$this -> update('users_notification_setting', array('data' => serialize($data)), 'uid = ' . intval($uid));
		}
	...
	public function update_notification_unread($uid) {
		return $this -> shutdown_update('users', array('notification_unread' => $this -> count('notification', 'read_flag = 0 AND recipient_uid = ' . intval($uid))), 'uid = ' . intval($uid));
	}

	public function update_question_invite_count($uid) {
		return $this -> update('users', array('invite_count' => $this -> count('question_invite', 'recipients_uid = ' . intval($uid))), 'uid = ' . intval($uid));
	}

	public function update_notic_unread($uid) {
		return $this -> shutdown_update('users', array('notice_unread' => $this -> count('notice_recipient', "recipient_uid = " . intval($uid) . " AND recipient_time = 0 AND (recipient_del IS NULL OR recipient_del = 0)")), 'uid = ' . intval($uid));
	}
	...
	//手机号码验证规则
	public function check_mobile_char($mobile) {

		$length = strlen(convert_encoding($mobile, 'UTF-8', 'GB2312'));
		if ($length <> 11) {
			$flag = true;
		}
		if (!preg_match('/^1(3|5|8)\d{9}$/', $mobile) OR $flag) {
			return AWS_APP::lang() -> _t('请输入11位手机号码');
		}
		return false;
	}
		...
		foreach ($action_list as $key => $val) {
			...
			switch ($val['associate_type']) {
				case ACTION_LOG::CATEGORY_QUESTION :
					...
					if (in_array($val['associate_action'], array(ACTION_LOG::ADD_TOPIC, ACTION_LOG::MOD_TOPIC, ACTION_LOG::MOD_TOPIC_DESCRI, ACTION_LOG::MOD_TOPIC_PIC, ACTION_LOG::DELETE_TOPIC, ACTION_LOG::ADD_TOPIC_FOCUS, ACTION_LOG::DELETE_TOPIC_FOCUS, ACTION_LOG::ADD_TOPIC_PARENT, ACTION_LOG::DELETE_TOPIC_PARENT)) AND $val['associate_attached']) {
		...
		foreach ($action_list as $key => $val) {
			...
			switch ($val['associate_type']) {
				case ACTION_LOG::CATEGORY_QUESTION :
					...
					if (in_array($val['associate_action'], array(ACTION_LOG::ANSWER_QUESTION)) AND $question_info['answer_count'] > 0) {
	...
	public function get_user_recommend_v2($uid, $limit = 10) {
		...
		if ($users_focus = $this -> query_all("SELECT DISTINCT friend_uid, fans_uid FROM " . $this -> get_table('user_follow') . " WHERE fans_uid IN (" . implode(',', $follow_uids) . ") ORDER BY follow_id DESC", $limit)) {
			foreach ($users_focus as $key => $val) {
				...
				$users_ids_recommend[$val['friend_uid']] = array('type' => 'friend', 'fans_uid' => $val['fans_uid']);
			}
		}
		...
		if ($topic_focus_uids = $this -> get_users_list_by_topic_focus($my_focus_topics_ids)) {
			foreach ($topic_focus_uids as $key => $val) {
				...
				$users_ids_recommend[$val['uid']] = array('type' => 'topic', 'topic_id' => $val['topic_id']);
			}
		}
	...
	function get_avatar($uid, $size = 'min', $return_type = 0) {
		$size = in_array($size, array('max', 'mid', 'min', '50', '150')) ? $size : 'real';
	...
	function update_thanks_count($uid) {
		...
		return $this -> update('users', array('thanks_count' => ($answer_counter + $question_counter)), "uid = " . intval($uid));
	}
	...
	function add_user_group($group_name, $type, $reputation_lower = 0, $reputation_higer = 0, $reputation_factor = 0) {
		return $this -> insert('users_group', array('type' => intval($type), 'custom' => 1, 'group_name' => $group_name, 'reputation_lower' => $reputation_lower, 'reputation_higer' => $reputation_higer, 'reputation_factor' => $reputation_factor, ));
	}

	function delete_user_group_by_id($group_id) {
		$this -> update('users', array('group_id' => 4, ), 'group_id = ' . intval($group_id));
	...
	function update_user_reputation_group($uid) {
		...
		if ($user_group['custom'] == 1) {
			if ($user_info['reputation_group']) {
				$this -> update_users_fields(array('reputation_group' => 0), $uid);
			}
		...
		if ($reputation_group != $user_info['reputation_group']) {
			return $this -> update_users_fields(array('reputation_group' => $reputation_group), $uid);
		}
	...
	function update_url_token($url_token, $uid) {
		return $this -> update('users', array('url_token' => $url_token, 'url_token_update' => time()), 'uid = ' . intval($uid));
	}
	...
	function forbidden_user($uid, $status, $admin_uid) {
		...
		$this -> model('account') -> update_users_fields(array('forbidden' => intval($status)), intval($uid));

		return $this -> insert('users_forbidden', array('uid' => intval($uid), 'status' => intval($status), 'admin_uid' => $admin_uid, 'add_time' => time(), ));
	}
	...
	public function set_default_timezone($time_zone, $uid) {
		return $this -> update('users', array('default_timezone' => htmlspecialchars($time_zone)), 'uid = ' . intval($uid));
	}
	...
	public function send_delete_message($published_uid, $question_content, $question_detail) {
		...
		$this -> model('email') -> action_email('QUESTION_DEL', $published_uid, get_js_url('/inbox/'), array('question_title' => $question_content, 'question_detail' => $question_detail, ));
	...
	public function save_recent_topics($uid, $topic_title) {
		...
		return $this -> update('users', array('recent_topics' => serialize($recent_topics)), 'uid = ' . intval($uid));
	}
